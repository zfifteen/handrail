...
Handrail is a free, local-first iOS remote control for Grok Build sessions on the user's Mac.

The product is useful because Grok Build remains local and authoritative. Handrail lets the user see, continue, supervise, approve, deny, and stop Grok work from iPhone or iPad without turning the project into a cloud workspace or generic remote shell.

## What Handrail Is

- A local iPhone and iPad companion for Grok Build on the user's Mac.
- A local WebSocket client for the Handrail CLI running on the user's network.
- A supervisor for Grok Build sessions, tool approvals, input, notifications, and status.
...
- A client for Claude, Gemini, OpenCode, Codex Desktop, or any non-Grok agent.
- An iOS file editor that bypasses Grok Build.
...
- App Store metadata and screenshots must describe a local Grok Build remote, not a cloud IDE, account product, generic terminal, or multi-agent workspace.
...
- Protocol changes must preserve local Grok Build ownership and must not silently drop unknown CLI/iOS message drift.
...
- Preserved free, local-first, Grok Build-only Handrail: yes/no.
