# Handrail Chat UI Contract

> Filename retains `codex-desktop` from the pre-revival era. **Grok Build** is now the source of truth for chat behavior and transcript shape.

Handrail chat must mirror the Grok Build session interaction model on iPhone as closely as the smaller screen permits.

This is not a request for a nicer Handrail-branded chat. This is not a request for a generic mobile chat redesign. This is a product requirement: the chat screen is a mobile controller for Grok Build sessions, so Grok Build is the visual and behavioral source of truth.

## Source Of Truth

The implementation target is Grok Build chat (ACP sessions + `updates.jsonl` transcripts).
...
- Grok output and user input appear as conversation turns, not dashboard cards.
- Thinking, files, diffs, approvals, and running status appear as compact conversation artifacts.
...
Any deviation from Grok Build must be caused by an iPhone constraint and documented in the implementation notes.
...
- Dashboard-style status panels such as a large `Grok is working` block occupying the composer/status region.
- A header whose controls or blur treatment consume transcript space without matching Grok Build behavior.
- A color palette driven by Handrail accent styling instead of neutral transcript styling.
...
The simulator captures from May 8, 2026 (Codex Desktop era) show the contract violation:
...
A chat redesign is acceptable only when a side-by-side review against Grok Build shows the same interaction model:
...
- A chat requiring approval, if a live Grok ACP approval fixture is available.
