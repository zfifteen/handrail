...
- #28 iPhone metadata and screenshot package. Listing copy exists at `store-assets/metadata.txt`, the support URL points at the public GitHub issue tracker, the privacy URL points at the public GitHub-rendered policy, and the marketing URL is deliberately omitted for v1. The screenshot plan exists at `store-assets/screenshot-plan.md`; the four required v1 iPhone screenshots are still missing. Approval-response copy and the approval screenshot are deferred until #2 has first-class approval-routing evidence.
