...
Triggered QA runs perform targeted validation from `$CODEX_HOME/automations/handrail-qa-lead/handoff.md` first. The handoff determines the smallest evidence needed for that run.
...
The daily sweep must leave evidence (commands + device targets + screenshots) in `test-artifacts/`. Triggered QA reports should name whether they performed targeted validation only or invoked broader simulator coverage.
